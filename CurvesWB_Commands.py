...
class s2rCommandFoil():
	...
	def GetResources(self):
		...
		return {'Pixmap'  : os.path.expandvars("/home/$USER") + ("/.FreeCAD/Mod/airFoil_tools/Resources/icons/s2rCommandFoil.png"), # the name of a svg file available in the resources
			...
			'ToolTip' : "airFoil2Rails: Uses Curves WB sketchOn2Rails functions for airFoil generation"}

	def Activated(self, sel=[]):

		self.sel = FreeCADGui.Selection.getSelection()
		try:
			if len(self.sel)<5 and len(self.sel)>1:
				for i in self.sel:
	...
	#highList
				highList = auxFunctions.geoForRails(highList)
				auxFunctions.foilToRails(highList,"High")			
	#lowList
				lowList = auxFunctions.geoForRails(lowList)
				auxFunctions.foilToRails(lowList,"Low")
		except:
